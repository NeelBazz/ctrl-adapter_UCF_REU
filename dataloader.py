...
class VideoLoader(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        self.audio_embedding_path = audio_embedding_path # Added
    ...
    def get_audio_embedding(self, video_path):
        folder_name = os.path.basename(os.path.dirname(video_path))  # e.g., video1
        base_name = os.path.splitext(os.path.basename(video_path))[0]  # e.g., clip_000
        emb_path = os.path.join(self.audio_embedding_path, folder_name, base_name + ".pt")

        if not os.path.exists(emb_path):
            raise FileNotFoundError(f"Audio embedding not found for {video_path}")

        data = torch.load(emb_path)
        return data["embedding"]  # shape: [1024]
    ...
    def __getitem__(self, index):
        ...
        #text = "" if self.use_empty_prompt else vid_prompt
                # Load audio embedding (replace text prompt)
        audio_embedding = self.get_audio_embedding(vid_path)
        ...
        if self.model_name == 'i2vgenxl':
            ...
            return {
                ...
                "audio_embedding": audio_embedding, #Replaces input_ids
